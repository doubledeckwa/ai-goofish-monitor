"""
Xianyu account management routing
"""
import json
import os
import re
...
def _validate_name(name: str) -> str:
    ...
    if not trimmed or not ACCOUNT_NAME_RE.match(trimmed):
        raise HTTPException(status_code=400, detail="Account name can only contain letters and numbers、Underline or dash。")
    return trimmed
...
async def create_account(data: AccountCreate):
    ...
    return {"message": "Account has been added", "name": account_name, "path": path}
...
async def update_account(name: str, data: AccountUpdate):
    ...
    async with aiofiles.open(path, "w", encoding="utf-8") as f:
        await f.write(data.content)
    return {"message": "Account has been updated", "name": account_name, "path": path}
